...
def write_health() -> None:
    """Write a health check timestamp to /tmp."""
    try:
        Path("/tmp/inference_health").write_text(str(time.time()))
    except OSError:
        pass


def main() -> None:
    """Run YOLO inference loop with MQTT publishing."""
                ...
                detections.append(
                    {
                        ...
                        "bbox": [
                            round(float(x), 1) for x in box.xyxy[0].tolist()
                        ],
                    }
                )
